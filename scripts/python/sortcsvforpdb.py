...
INPUT_DIR = REPO_ROOT / "pdbpred"
OUTPUT_FILE = REPO_ROOT / "data" / "processed" / "sorted_scorespdb.csv"
...
def main():
    ...
    for file in csv_files:
        ...
        if score is not None and probability is not None and score > 30:
            data.append((file.name, score, probability))
...
def extract_first_score_and_probability(csv_file: Path):
    try:
        df = pd.read_csv(csv_file)
        if not df.empty:
